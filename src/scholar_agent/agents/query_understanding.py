...
def _extract_keyword_terms(query: str, venues: list[str]) -> list[str]:
    ...
    terms.extend(venues)
    # Chinese mappings are appended after token filtering, so apply the
    # same stopword policy once more.  This avoids emitting generic mapped
    # words such as "papers" as if they were a scientific must-have term.
    return _dedupe(
        term for term in terms if term and term.casefold() not in STOPWORDS
    )
