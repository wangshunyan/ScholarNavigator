...
- P0 精确 arXiv ID 语料契约和 Faiss 构建器已实现，但当前 checkout 缺少可审计的完整 P0/Faiss 运行产物；不得把旧文档数字当作成绩，待新输入和成对评测完成后再填写。
...
2. P0/Faiss 变更后的 200 条资格门禁尚未在当前 checkout 形成可审计成对产物；完成前不得启动或宣称 1000 条候选成绩。
...
- 旧 `local_hybrid` 数字来自 legacy 标题匹配语料和不一致代码指纹，不能作为新主线成绩；当前 P0/Faiss 结果尚未形成可审计新运行目录。
