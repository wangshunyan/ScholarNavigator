...
6. `contest_qual200_dense_reranker_soft_v2` 已通过 200 条配对资格门禁，且完整运行 `contest_full_dense_reranker_soft_v2` 已启动；在完整运行自然结束并通过真实 reranker、资源账本与结果完整性审计前，不得把 soft Judgement 写为正式成绩。
7. 将新主线的 `local_bm25` 与 `local_hybrid` 对比、阶段诊断中的初始候选 Recall、Judgement FN 和平均延迟写入说明书。
8. 填写团队、学校、指导教师、成员分工、软件著作权/开源许可证等竞赛表单字段，并完成匿名要求检查。
9. 录制 3 到 5 条交互式演示，确认视频没有 API Key、本地绝对路径或无关个人信息。
