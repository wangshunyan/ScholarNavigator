...
| dense + reranker + fixed RRF + soft Judgement | `contest_qual200_dense_reranker_rrf_soft_v3` | 已完成并通过资格门禁 | qualification only | qualification only | passed | 200/200、零失败、零 fallback；相对 reranker v4 的 F1@20/Recall@20 paired-bootstrap 95% CI 均严格为正。 |
| dense + reranker + fixed RRF + soft Judgement | `contest_full_dense_reranker_rrf_soft_v3` | 已完成并审计通过 | 0.02726 | 0.16817 | passed | MRR 0.09834，平均延迟 4.022 s；1000/1000、零失败、零 fallback，GPU1 reranker 审计通过。内部指标不等同赛事官方 scorer。 |
...
6. `contest_qual200_dense_reranker_rrf_soft_v3` 的 paired-bootstrap 支持质量提升，并证明初始候选 Recall 未下降、Judgement false-negative rate 下降；对应 full RunId 已完成独立 1000 条运行、完整性检查、资源账本和 reranker 审计。
7. `contest_full_dense_reranker_rrf_soft_v3` 已完成 1000 条并通过完整性、资源账本和 GPU reranker 审计；F1@20=0.02726023、Recall@20=0.16817491、MRR=0.09833638、平均延迟=4.022 s，reranker P50/P95=0.748/0.896 s。以上仍为内部工程指标，不等同赛事官方 scorer。

## 后续正式实验口径

P0/Faiss 语料、索引和 Dense 完整运行已经完成。当前可引用完整成功且审计通过的 rules、Dense、reranker v4 和 fixed RRF + soft Judgement v3；LLM 仍未通过正式门禁：
