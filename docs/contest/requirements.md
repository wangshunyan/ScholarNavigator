...
根据赛事公告公开的《初赛作品提交规范》（页面附件，核对日期 2026-08-23）：

- 项目说明书：PDF，300 页以内；文件名按“团队名称_项目名称_项目说明书.pdf”。
- 项目文档：使用官方模板填写，PDF；文件名按“团队名称_项目名称_项目文档.pdf”。
- 项目视频：MP4，不超过 5 分钟且不超过 200 MB；文件名按“团队名称_项目名称_项目视频.mp4”。
- 项目源码及其他资料：ZIP，不超过 200 MB；文件名按“团队名称_项目名称_源码.zip”。
- 公告截止时间：2026-09-01 23:59；以官网提交页面的最终要求为准。

仓库的 source-only 发布包只负责可复现源码与轻量离线演示，不替代说明书、视频或平台提交。当前 Git 跟踪文件约 273 MB，若直接压缩可能超过 200 MB；提交前必须运行发布构建器并检查 ZIP 实际大小，必要时将 92 MB 的标题型 BM25 语料改为单独按要求准备的外部数据/下载说明，不能通过删除代码或压缩后猜测大小来满足限制。
