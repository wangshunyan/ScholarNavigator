...
产品默认来源仍固定为 OpenAlex、arXiv、Semantic Scholar 和 PubMed；`local_bm25` 和 `local_hybrid` 只在调用方同时显式选择来源并提供语料/字段配置时注册到本次运行。`local_bm25` 使用 SQLite FTS5 的确定性 Unicode tokenizer，按 BM25 分数和文档 ID 稳定排序。`local_hybrid` 复用同一 BM25 语料，并额外读取带摘要的公开语义子集，使用本地 SentenceTransformers/BGE 模型生成归一化向量；查询时对 BM25 与语义候选做 Reciprocal Rank Fusion。磁盘索引缓存指纹包含语料 SHA-256、字段映射、tokenizer/connector 版本、模型指纹和向量维度；不同语料、模型或字段配置不能共享 Snapshot key。连接器不接收 Dataset Adapter、qrels、gold、crosswalk 或 case ID，返回的身份只能来自配置字段或配置为稳定身份的原始文档 ID。
