...
- **任务编号**：P1-01；**独立提交**：开放全文 fixture、解析器和结果证据映射分步提交。
...
- **验收标准**：仅接受许可明确或公开可核验来源；段落顺序、来源 URL、内容哈希和定位稳定；下载/解析失败保留摘要结果并记录证据缺失；不抓取受限全文。
- **自动化验证方式**：本地 HTML/PDF/text fixture、重复解析哈希测试、许可拒绝测试、失败闭合测试和 API 映射测试。
- **可用性与阻塞**：解析器与失败路径可离线完成；真实开放 URL、许可元数据和可能的官方/出版 API 需要外部数据，缺失时不得标记完成。
- [ ] 未开始
