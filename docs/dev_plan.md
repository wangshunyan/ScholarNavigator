...
- **外部环境复核（2026-08-21）**：旧服务器具备目标 Linux/x86_64/Python 3.12 环境，但工作树未清理，按同步规则不能进入项目目录生成 lock 或 wheelhouse。发布候选合同引用的历史 commit 也不在本地对象库，GitHub 按该对象 ID 获取被拒绝；这两项仍为外部证据/环境 blocker。
