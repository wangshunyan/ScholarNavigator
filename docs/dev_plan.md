...
- 当前代码提交：以 `git rev-parse HEAD` 为准；本地 `main` 与 `origin/main` 一致，发布 smoke 的 `source_commit` 是权威绑定。当前提交为 `04490cc`；本轮演示可访问性修复后的 clean-clone smoke 已重新生成并绑定该提交。
...
- **P2-02 发布 smoke 复核（2026-08-25，本轮）**：使用项目 `.venv` 运行 `scripts/check_clean_clone_smoke.py`，结果 `status=ready`；health/config=200，离线 BM25=5 条，网络请求=0，LLM=disabled，gold/qrels 未加载；source-only 包 1,030 个文件、36,987,528 bytes，包内 `source_commit=04490cc`，验证器通过。该结果仍不替代正式赛事 scorer 或 P1-02 资格评测。
