...
- 当前公开提交为 `78db2048a0dd4cbbdddb7746e09684b105b3a676`（`78db204`），已推送 GitHub；本地仅有 Git 忽略的实验资产变动。服务器原目录仍在 `eb7d151` 且有未提交内容，因此不在原目录直接启动新实验或覆盖文件；隔离服务器 worktree 已同步至 `78db204` 且干净（真实路径不进入发布包）。
...
- [x] **P0-01 当前提交的全量回归与前端构建**
  ...
  - 验收：稳定基线提交 `373e5e2` 的全量后端回归为 `2348 passed, 184 skipped, 2 warnings`；clean-clone 严格测试通过；`frontend` lint/build 通过；5 条 gold-blind demo 5/5 成功、网络 0、LLM 0、gold/qrels 未加载。当前 `78db204` 已重跑前端 lint/build 与 12 项全文安全回归；提交前仍应在可控 CI/空闲执行环境重跑全量后端回归。
  - 依赖：锁定的本地 `.venv` 与 `frontend/node_modules`。

- [x] **P0-02 隔离服务器实验工作区**
  - 目标：不改动服务器原有 dirty 工作树，创建基于当前公开提交的独立工作目录，并记录其 commit、依赖与 v3 资产校验。
  - 验收：2026-08-25 已创建独立服务器 worktree，当前发布代码固定 `78db204` 且干净；v3 语料 SHA-256、索引指纹 `91302a92…e68f71`、模型指纹 `926ed116…fdf1b7`、569,432 篇和 ANN Recall@10=0.984 均复核；新 worktree 专项回归 `23 passed`；原目录未修改。服务器使用受控的既有依赖运行时，隔离 worktree 不复制模型、索引或环境凭据。
