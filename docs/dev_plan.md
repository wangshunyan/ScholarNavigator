...
- 当前代码提交：以 `git rev-parse HEAD` 为准；本地 `main` 与 `origin/main` 一致，发布 smoke 的 `source_commit` 是权威绑定。当前提交为 `5eec45e`；本轮 Hybrid runtime-config fixture 修复后的 clean-clone smoke 已重新生成并绑定该提交。
...
- **P2-02 发布 smoke 复核（2026-08-25，本轮）**：使用项目 `.venv` 运行 `scripts/check_clean_clone_smoke.py`，结果 `status=ready`；health/config=200，离线 BM25=5 条，网络请求=0，LLM=disabled，gold/qrels 未加载；source-only 包 1,030 个文件、36,987,939 bytes，包内 `source_commit=5eec45e`，验证器通过。该结果仍不替代正式赛事 scorer 或 P1-02 资格评测。
...
- **P2-03 当前证据版说明书（2026-08-25，本轮）**：新增 `docs/contest/submission-report-current.md`，只引用当前可读取的 v3 200 条内部工程指标、元数据哈希、发布 smoke 和已知限制；不填官方成绩、不隐藏 venue/DOI 缺口、不把 Reranker 或 LLM 写成默认收益。发布 smoke 在新提交 `0a19b00` 上重新通过：1,031 文件、36,991,394 bytes、manifest/source commit 一致。
