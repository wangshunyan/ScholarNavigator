...
- **P1-04 可执行子目标（2026-08-24）**：修复合成器只接受 title/abstract/venue/metadata、忽略已验证全文段落的缺口。现在仅将 `license_verified=true` 且带稳定段落定位的全文证据加入 synthesis evidence table；未验证全文仍 fail-closed。固定同一 RankedPaper 的前后成对测试显示 rank/final_score 不变，全文版本新增 `full_text` 证据行并移除“全文不可用”限制；相关全文/API/合成/质量专项 `48 passed, 1 warning`。这提升证据展示，不代表全文覆盖率或 Evidence F1 已完成。
