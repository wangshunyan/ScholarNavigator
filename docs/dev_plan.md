...
- **P2-02 演示可靠性修复（2026-08-25，本轮）**：阶段卡片现在同时使用 SSE 事件和轮询返回的 `current_stage`，事件连接短暂丢失时仍能正确显示进行中阶段；每个阶段补充可访问状态文本和 `aria-live` 当前阶段播报。该改动不改变检索、排序、预算或结果结构；前端 lint/build 已通过。
- **P2-02 发布 smoke 复核（2026-08-25，本轮）**：提交 `074e221` 后使用项目 `.venv` 运行 `scripts/check_clean_clone_smoke.py`，结果 `status=ready`；health/config=200，离线 BM25=5 条，网络请求=0，LLM=disabled，gold/qrels 未加载；source-only 包 1,031 个文件、约 36.99 MB，manifest/source commit 由 smoke 运行时自动绑定并验证通过。该结果仍不替代正式赛事 scorer 或 P1-02 资格评测。
...
- **P2-02 结果可审计性补强（本轮）**：结果导出区现在显式展示后端 `run_id`，使评委可将页面结果、导出文件和脱敏运行证据对应起来；不改变检索、排序、预算或导出数据结构。前端 lint/build 通过，提交后需重新运行 clean-clone smoke。
- **P2-02 结果可审计性验证（本轮）**：在包含运行 ID 展示的提交上，clean-clone smoke 返回 `status=ready`；health/config=200、离线 BM25=5 条、网络请求=0、LLM disabled、gold/qrels 未加载，source-only 包约 36.99 MB，发布包验证通过。
- **P2-02 指标范围可见性（本轮）**：结果导出区现在同时明确提示“工程/演示运行，不是赛事官方 scorer 成绩”，与报告和 API 的内部指标边界一致；不改变结果或导出结构。前端 lint/build 通过，待提交后重跑发布 smoke。
- **P2-02 失败路径可审计性（本轮）**：运行进度面板现在在 queued/running/failed/cancelled/succeeded 全部状态持续展示 `run_id`，失败或取消时也能对应后端诊断和证据记录；前端 lint/build 通过，不改变检索逻辑，待提交后重跑发布 smoke。
- **P2-02 导出一致性复核（本轮）**：JSON 导出顶层、Markdown 首段和下载文件名均已绑定同一 `run_id`；无需改变导出 schema，避免页面与文件出现不同运行身份。该结论由当前 `frontend/src/lib/export.ts` 代码复核确认。
- **服务器 v3 证据审计（2026-08-24，本地已读取脱敏资产）**：`outputs/server_assets/20260824_metadata_v3/` 中三组运行的最新 generation 均有 `RUN_COMPLETED`/`COMMITTED`，`run_manifest` 为 `completed=200/200`，结果 200 条、失败 0；baseline、Reranker 和候选数 120 运行共享语料 SHA-256=`7a385c87…08a31a`、569,432 篇和同一索引指纹。v3 语料结构审计通过且 arXiv ID 唯一，但 venue 完整度仅 `12.152%`、DOI `17.897%`，所以 P1-01 元数据门禁仍未完成。
- **服务器 Reranker 成对结论（2026-08-24）**：`contest_qual200_metadata_v3_hybrid_baseline_eb7d151` 与 `contest_qual200_metadata_v3_hybrid_reranker_eb7d151` 通过严格只差 Reranker 的配置/代码/查询门禁；candidate 审计为 CUDA、fallback=0、batch=8、候选=120、593 次成功推理、P50/P95 延迟约 `1.761/2.077s`、峰值显存约 `1.50GB`。配对 bootstrap 5,000 次：ΔRecall@20=`-0.00054`（95% CI `[-0.00987,0.00838]`）、ΔF1@20=`+0.00029`（`[-0.00219,0.00273]`）、ΔF1@10=`-0.00475`（`[-0.00875,-0.00143]`）；没有任何指标满足严格正向门禁，Reranker 保持默认关闭。
- **服务器候选数 120 负向消融（2026-08-24）**：相同 v3 资产下把 BM25/语义候选从 60 提至 120，ΔRecall@20=`-0.00400`（95% CI `[-0.02083,0.01133]`）、ΔF1@20=`-0.00051`（`[-0.00290,0.00174]`）；不启用该配置。上述均为内部离线指标，不是赛事官方成绩；脱敏成对报告保存在 ignored `outputs/local_audit_20260824/`。
- **判定误杀离线诊断（2026-08-24，本轮）**：新增 `scripts/analyze_judgement_false_negatives.py`，从已完成 v3 baseline 的 gold 诊断与 committed generation delta 中恢复 31 条“已检索但被 judgement 判为 weakly/irrelevant”的完整 feature vector；结果为 weakly=22、irrelevant=9，向量找到率 31/31。累计扣分主要来自 `constraint_coverage_adjustment=-1.86`（每条均为 `-0.06`），而不是模型/GPU；同时可见大量 query 叙述词被当作 must-have（如 `only/its/called/finding`）。产物为 ignored `outputs/local_audit_20260824/judgement_false_negative_diagnostic.json`。该脚本只做检索完成后的离线审计，不改变在线检索、排序或默认规则；下一步仅对明确的查询约束抽取缺陷设计最小修复，并先做 30 条筛选成对实验。
- **v3 资产可移植性修复（2026-08-24，本轮）**：发现导入服务器索引时，`local_hybrid` 会把索引构建机的绝对 `model_path` 当作硬约束，即使本机模型内容相同也拒绝启动。现改为只校验模型内容 `model_fingerprint`，保留持久化路径作为 provenance；新增跨机器路径回归测试。该改动不改变向量、排序或候选预算；服务器 v3 模型指纹与本地模型指纹仍不同（`926ed116…` vs `f95b930a…`），因此不会把两者误认为同一模型，正式比较仍需同一指纹资产。
- **查询清理成对复核（2026-08-24）**：在同一 v3 Hybrid 语料/索引、同一前 200 条查询、`high_recall`/300 候选、`top_k=20`、单 worker、零网络/零 LLM 下，对移除叙述性停用词的 candidate 做 5,000 次 query-level bootstrap。candidate 已完成 200/200，失败 0；ΔRecall@20=`0.00000`（95% CI `[0.00000,0.00000]`）、ΔF1@20=`0.00000`（`[0.00000,0.00000]`）、ΔF1@10=`-0.00071`（`[-0.00214,0.00000]`）、ΔF1@5=`+0.00167`（`[0.00000,0.00500]`）。未达到“Recall@20 或 F1@20 均值为正且 CI 下界大于 0”的质量门禁，因此不把该改动宣传为检索质量提升，也不改变默认排序策略；保留它作为查询理解可解释性/演示修复。报告位于 ignored `outputs/benchmark_runs/querycleanup_paired_analysis.json`，所有数值均为内部离线指标、不是赛事官方成绩。
