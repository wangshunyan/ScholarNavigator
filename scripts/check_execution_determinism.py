#!/usr/bin/env python3
"""Run the offline execution_determinism_v1 gate."""
...
DEFAULT_PROTOCOL = ROOT / "benchmark" / "execution_determinism_v1_protocol.json"


class _ArgumentParser(argparse.ArgumentParser):
    def error(self, message: str) -> None:
        raise ExecutionDeterminismError(f"usage_error:{message}")


def _parser() -> argparse.ArgumentParser:
    parser = _ArgumentParser(
        description="Validate cross-execution determinism without network access."
    )
    ...
    check.add_argument(
        ...
        help="Deterministic fault injection for gate verification only.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    except ExecutionDeterminismError as exc:
    ...
    except FixtureNotEligible as exc:
        report = _error_report("not_eligible", EXIT_NOT_ELIGIBLE, str(exc))
        _emit(report, output)
        return EXIT_NOT_ELIGIBLE
    except (ExecutionDeterminismError, OSError, ValueError) as exc:
        report = _error_report("usage_error", EXIT_USAGE_ERROR, str(exc))
        _emit(report, output)
        return EXIT_USAGE_ERROR
